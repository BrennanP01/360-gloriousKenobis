...
func main() {
	...
	platforms := [4]string{"quit", "twitter", "youtube", "facebook"}
	...
	for selPlatform != 0 {
		...
		if selPlatform != 0 {
			...
			url = localUrl + "?plat=" + platforms[selPlatform] + "&hash=" + hastag + "&time=" + timezones[tz]
			//url = prodUrl + "&plat=" + platforms[selPlatform] + "&hash=" + hastag + "&time=" + timezones[tz]
			//url = prodUrl
			clientConnect(url)
		}
	}
...
/***************************************************************************
* This function sends a gett message to the url passed as a parameter and  *
...
func clientConnect(url string) {
	var auxStr [5]HashtagSt			// For testing purpose. Remove on production system

// Section disabled for testing purpose. Enable on production system
	client := &http.Client{}
...
// For testing purpose. Remove on production system. Remove up to label MVRC 
	//auxStr[0] = HashtagSt {Hashtag: "hastag1", Occur: 16}
	//auxStr[1] = HashtagSt {Hashtag: "hastag2", Occur: 6}
	//auxStr[2] = HashtagSt {Hashtag: "hastag3", Occur: 10}
	auxStr[0] = HashtagSt {"hastag1", 16}
	auxStr[1] = HashtagSt {"hastag2", 6}
	auxStr[2] = HashtagSt {"hastag3", 10}
	//q := WSst {Platform: "twitter", Time: 5, Hashtags: auxStr[0:3]}
	q := WSst {"twitter", 5, auxStr[0:3]}
	json_data, err := json.Marshal(q)
	fmt.Printf("JSON Answer: %s\n", json_data)
	w := WSst{}
	err = json.Unmarshal(json_data, &w)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("Len: %d\n", len(w.Hashtags))	
...
func validateAnswer(question string, high int) int {
	...
	for err != nil {
		...
		if err != nil || answer < 0 || answer > high {
			...
			fmt.Scanf("%s", &str)
			// To cover the case that the conversion is right but the number  is out of the expected range
			err = FoundErr
		}
	}
